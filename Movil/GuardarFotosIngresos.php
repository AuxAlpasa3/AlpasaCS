...
try {
    $sqlEncabezado = $Conexion->prepare("SELECT IdFotografias 
                                        ...
                                        WHERE IdTarja = ? AND Tipo = 1 AND Almacen = ?");
...
$uploadDir = "C:\\xampp\\htdocs\\{$RutaLocal}\\Ingresos\\Fotografias\\{$directoryName}\\";
...
try {
    ...
    // Insertar en base de datos de manera optimizada (Tipo 1 para Ingresos)
    $sql = "INSERT INTO t_fotografias_detalle 
       (IdFotografiaRef, NombreFoto, RutaFoto,NextIdFoto)
        VALUES (?, ?, ?, ?)";

    $stmt = $Conexion->prepare($sql);
    $rutaPublica = "https://intranet.alpasamx.com/{$RutaLocal}/Ingresos/Fotografias/{$directoryName}/{$nombreFoto}";
