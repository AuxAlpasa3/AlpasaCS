...
$Cantidad =  1; 
$TipoImpresora = 'Etiquetas';

// Obtener información de la impresora
$SentiImpresora = $Conexion->prepare("SELECT NombreImpresora FROM t_impresoras WHERE TipoImpresora = 'Etiquetas' AND Almacen = :idAlmacen");
$SentiImpresora->bindParam(':idAlmacen', $IdAlmacen, PDO::PARAM_INT);
$SentiImpresora->execute();
$Impresora = $SentiImpresora->fetch(PDO::FETCH_COLUMN); // Cambiado a fetch para obtener un solo valor

if (empty($Impresora)) {
    die("Error: No se encontraron Impresoras disponibles");
}

$NombreImpresora = $Impresora; // FALTABA ESTA ASIGNACIÓN

// Obtener códigos de barras
$sentiCodigos = $Conexion->prepare("SELECT CodBarras FROM t_ingreso WHERE 
IdTarja = :idTarja AND Almacen = :idAlmacen");
...
if (!file_exists($sumatraPath)) {
    ...
    if (!$sumatraFound) {
        // Limpiar archivo temporal antes de morir
        unlink($tempPdf);
        die("SumatraPDF no encontrado. Verifique la instalación.");
    }
}
...
if (strpos($printers, $NombreImpresora) === false) {
    // Limpiar archivo temporal antes de morir
    unlink($tempPdf);
    die("La impresora '$NombreImpresora' no está instalada o no existe");
}

// Intentar impresión con SumatraPDF
$command = '"' . $sumatraPath . '" -print-to "' . $NombreImpresora . '" "' . $tempPdf . '"';
    ...
    <script>
        document.addEventListener('DOMContentLoaded',function(){
            Swal.fire({
                ...
                text: 'Código de error: $return_var',
