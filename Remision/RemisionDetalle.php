  ...
  <script type="text/javascript">
    $(document).ready(function () {
      const table = $('#dataTableDetalle').DataTable({
        ...
        "serverSide": true,
        "ajax": {
          "url": "ProcesoRemDetalle/ObtenerRemisionDetalle.php",
          "type": "POST",
          "data": function (d) {
            ...
            return `
                                <div style="text-align: center;">
                                    <button type="button" class="btn-editar btn btn-warning btn-sm btn-action" 
                                            ...
                                            data-idarticulo="${row.IdArticuloNum}">
                                        <i class="fa fa-pen"></i>
                                    </button>
      ...
      function reinicializarEventos() {
        $(document).off('click', '.btn-editar').on('click', '.btn-editar', function () {
          ...
          cargarModalEditar(idLinea, idRemision, idArticulo);
        });
        ...
        $(document).off('click', '.btn-eliminar').on('click', '.btn-eliminar', function () {
          ...
          var idRemision = $(this).data('idremision');
          var idArticulo = $(this).data('idarticulo');
          cargarModalEliminar(idLinea, idRemision, idArticulo);
        });
        ...
        // Evento para el nuevo botón de Movimientos Históricos
      ...
      function cargarModalAgregar() {
        $('#modal-container').load(
          'ProcesoRemDetalle/Agregar.php?IdRemision=<?php echo $IdRemision; ?>&IdRemisionEncabezado=<?php echo $IdRemisionEncabezado; ?>&IdAlmacen=<?php echo $IdAlmacen; ?>',
          function () {
      ...
      function cargarModalEditar(idLinea, idRemision, idArticulo) {
        $('#modal-container').load('ProcesoRemDetalle/Modificar.php?IdLinea=' + idLinea + '&IdRemision=' +
          idRemision + '&IdArticulo=' + idArticulo +
          '&IdRemisionEncabezado=<?php echo $IdRemisionEncabezado; ?>&IdAlmacen=<?php echo $IdAlmacen; ?>',
          function () {
            $('#modificarDetalle').modal('show');

            setTimeout(function () {
              if (typeof inicializarEventosModal === 'function') {
      ...
      function cargarModalEliminar(idLinea, idRemision, idArticulo) {
        $('#modal-container').load('ProcesoRemDetalle/Eliminar.php?IdLinea=' + idLinea + '&IdRemision=' +
          idRemision + '&IdArticulo=' + idArticulo +
          '&IdRemisionEncabezado=<?php echo $IdRemisionEncabezado; ?>&IdAlmacen=<?php echo $IdAlmacen; ?>',
          function () {
            $('#eliminarDetalle').modal('show');
            $(document).off('click.modal-close').on('click.modal-close',
              '[data-dismiss="modal"], .btn-close, .modal-close',
              function () {
      ...
      // Función para cargar el modal de Movimientos Históricos
