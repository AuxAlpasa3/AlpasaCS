...
$IdRemisionAgrupada = (!empty($_POST['id'])) ? $_POST['id'] : NULL;
?>
  ...
  .search-container {
    ...
    <div class="content-wrapper">
            ...
            <div class="col-12">
              <div class="card">
                 <div class="card-header text-white" style="padding: 1rem; border-bottom: 2px solid #d94f00; background-color: #d94f00 ">
                  <h1 class="card-title">Remisión Cita para Salida: <b><?php echo "REM-AGR ".$IdRemisionAgrupada; ?></b> </h1>
                </div>
                <div class="card-body"> 
                  <div style="max-width: 100%;">
                    <div style="width: 100%;">
                        <div class="row">
                          <div class="col-12">
                            <?php
                             $sentSalidas = $Conexion->prepare("SELECT t2.IdLinea,t5.NombreCliente,t3.MaterialNo, Concat(t3.Material,' ',t3.Shape) as Articulo, t2.Piezas, t1.IdRemision, t2.Piezas-sum(isnull(t4.Piezas,0)) as Faltan,sum(isnull(t4.Piezas,0)) as totales
                                FROM t_remision_encabezado as t1 
                                LEFT JOIN t_remision_linea as t2 on t1.IdRemision=t2.IdRemision
                                LEFT JOIN t_articulo as t3 on t2.IdArticulo =t3.IdArticulo 
                                LEFT JOIN t_pasoSalida as t4 on t1.IdRemision=t4.IdRemision and t2.IdLinea=t4.IdLinea
                                INNER JOIN t_cliente as t5 on t1.Cliente=t5.IdCliente
                                where t1.TipoRemision=2 and t1.IdRemisionAgrupada=? and t1.Estatus in(0,1) group by t2.IdLinea,t3.MaterialNo,t3.Material,t3.Shape,t2.Piezas,t1.IdRemision,T5.NombreCliente");
                             $sentSalidas->execute([$IdRemisionAgrupada]);
                              ...
                              <div class="col-12">
                                <section class="pt-2">
                                  <div class="table-responsive">
                                    <form id="formLote" action="ProcesoSalidaRemisionAgrupada/SalidaAgrupada.php" method="POST" enctype="multipart/form-data">
                                      <input type="text" name="user" value="<?php echo $IdUsuario;?>" hidden>
                                      <input type="number" name="IdRemisionAgrupada" value="<?php echo $IdRemisionAgrupada;?>" hidden>

                                    <button type="submit" class="btn btn-danger" style="background-color: #d94f00;" id="DarSalida">Guardar</button>
                                      ...
                                      <thead>
                                        <tr>
                                         ...
                                         <th width="auto" style="color:black; text-align: center;">IdLinea</th>
                                      ...
                                      <tbody>
                                        ...
                                        <tr>
                                          <td width="auto" style="text-align: center;">
                                            ...
                                            <input type="radio" name="Elegir" value="<?php echo $Salida->IdLinea;?>" onclick="multipleActions(this.value,<?php echo $IdRemision; ?>,<?php echo $Salida->MaterialNo;?>,<?php echo $Salida->Faltan; ?>)" data-materialno="<?php echo $Salida->MaterialNo; ?>">
                                          </td>
                                          <td width="auto" style="text-align: center; ">
                                            <b><?php echo $Salida->IdRemision;?></b>
                                          </td>
                                          <td width="auto" style="text-align: center; ">
      ...
      <script type="text/javascript">
          ...
          const submitBtn = document.getElementById('DarSalida');
          ...
          fetch('ProcesoSalidaRemisionAgrupada/SalidaAgrupada.php', {
              method: 'POST',
              body: formData
          })
          ...
          function multipleActions(idLote,IdRemision,MaterialNo,faltan) {
            mostrarSeleccionados(idLote,IdRemision,MaterialNo);
            mostrarLote(idLote,IdRemision,MaterialNo,faltan);
            verificarFaltan(); 
          }

          function mostrarLote(idLote, IdRemision, MaterialNo, faltan) {
            fetch('../Salidas/obtenerlote.php?id=' + idLote + '&idRemision=' + IdRemision + '&MaterialNo=' + MaterialNo)
            .then(response => response.json())
            .then(data => {
                ...
                data.forEach(registro => {
                    ...
                    if (registro.EsArmado == 1) {
                        ...
                        codBarrasFormateado = `ALP-${String(registro.CodBarras).padStart(6, '0')}`;
                    }
                    ...
                    row.innerHTML = `
                        ...
                        <td style="text-align: center;">${registro.EstMaterial}</td>
                        <td style="text-align: center;">${registro.Piezas}</td>
                        <td style="text-align: center;" hidden="${!registro.EsArmado}">${registro.EsArmado}</td>
                      <td style="text-align: center;" hidden="${!registro.IdRemision}">${registro.IdRemision}</td>
          ...
          function mostrarSeleccionados(idLote, IdRemision, MaterialNo) {
            fetch('../Salidas/obtenerSeleccionados.php?id=' + idLote + '&idRemision=' + IdRemision + '&MaterialNo=' + MaterialNo)
                .then(response => response.json())
                .then(data => {
              ...
              data.forEach(elegidas => {
                  const codBarrasFormateado = `ALP-${String(elegidas.CodBarras).padStart(6, '0')}`;
                  const row = document.createElement('tr');
                  row.innerHTML = `
                      <td style="text-align: center;">${codBarrasFormateado}</td> 
          ...
          function añadirRegistros() {
            ...
            const EsArmado = fila.cells[10].textContent;
            const IdRemision = fila.cells[11].textContent;
            const EstadoMaterial = fila.cells[12].textContent; 
            ...
            if (EstadoMaterial == 8) {
                mostrarConfirmacionObsoleto().then((result) => {
                    if (result.isConfirmed) {
                        enviarRegistro(radio.value, IdRemision, CodBarras, MaterialNo, piece, EsArmado, Estatus, IdRemision, MaterialNo);
                    }
                });
            } else {
                enviarRegistro(radio.value, IdRemision, CodBarras, MaterialNo, piece, EsArmado, Estatus, IdRemision, MaterialNo);
            ...
            return Swal.fire({
                ...
                html: `
                    <div class="alert alert-warning" style="background-color: #d94f00; color:white;">
                        <i class="fas fa-exclamation-triangle"></i> Está intentando añadir un registro con estado OBSOLETO.
                    </div>
                    <p>¿Desea continuar con esta acción?</p>
                    <div class="form-group">
                        <label for="Usuario">Seleccionar al usuario:</label>
                         <?php  
                              $sentUsuario = $Conexion->query("SELECT IdUsuario,Usuario FROM t_usuario where TipoUsuario in(1,5);"); 
                              $Usuario = $sentUsuario->fetchAll(PDO::FETCH_OBJ);
                          ?>
                          <select class="form-control" name="Usuario" id="Usuario" required>
                              <?php foreach($Usuario as $Usuarios){ ?>
                                  <option value="<?php echo htmlspecialchars($Usuarios->IdUsuario); ?>">
                ...
                preConfirm: () => {
                    ...
                    return validarPasswordObsoleto(password,Usuario).then(esValida => {
                        if (!esValida) {
        ...
        function validarPasswordObsoleto(password, usuario) {
        ...
        function enviarRegistro(id, IdRemision, CodBarras, MaterialNo, piece, EsArmado, Estatus, idRemision, materialNo) {
            ...
            fetch('ProcesoSalidaRemisionAgrupada/AgregarRegistroBD.php', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
          ...
          function EliminarRegistro(idLote,IdRemision,CodBarras) {
              ...
              if (result.isConfirmed) {
                  ...
                  xhr.open("POST", "ProcesoSalidaRemisionAgrupada/EliminarRegistroBD.php", true);
          ...
          function verificarFaltan() {
            let todosCero = true;
            document.querySelectorAll('tbody tr td:nth-child(8) b').forEach(cell => {
                              ...
                              <div class="col-12">
                                <section class="pt-2">
                                  ...
                                  <div class="table-responsive">
                                    <table class="table table-bordered  table-striped" id="tablaLote" name="tablaLote"> 
                                      <thead>
                                        <tr>
                                          ...
                                          <th  style="color:black; text-align: center;">Estado Material</th>
                                    ...
                                    <button type="button"  class="btn btn-danger"  style=" background-color: #d94f00; !important;" id="btnAñadir" onclick="añadirRegistros()">Añadir</button>
                                  </div>
                                </section>
                              </div>
                            </div>
