    ...
    if ($Query6) {
        foreach ($Query6 as $rows) {
            $datosNoCoincidentes[] = array(
                'FolioIngreso2' => $row->FolioIngreso,
                'FechaIngreso2' => $row->FechaIngreso,
                'FechaProduccion2' => $row->FechaProduccion,
                'MaterialShape2' => $row->MaterialShape,
                'materilaNo2' => $row->MaterialNo,
                'piezas2' => $row->Piezas,
                'numPedido2' => $row->NumPedido,
                'CodBarras2' => 'ALP-' . sprintf("%06d", $row->CodBarras1),
                'origen2' => $row->Origen,
                'cliente2' => $row->Cliente,
                'paisOrigen2' => $row->PaisOrigen,
                'noTarima2' => $row->NoTarima,
                'idRemision2' => $row->IdRemision,
                'Comentarios2' => $row->Comentarios,
            );
        }
    }
