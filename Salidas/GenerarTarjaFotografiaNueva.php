...
$sentencia = $Conexion->prepare("SELECT t1.IdTarja, t1.IdRemision ,t2.Direccion,CONCAT(t2.Municipio,', ',t2.Estado,', ',t2.Pais) as Direccion2, t2.CodPostal,t2.NumRecinto
    FROM t_salida as t1 
    inner join t_almacen as t2 on t1.Almacen=t2.IdAlmacen
    WHERE IdTarja =? and t1.Almacen=? GROUP BY t1.IdTarja, t1.IdRemision ,t2.NumRecinto,t2.Direccion,t2.Municipio,t2.Estado,t2.Pais,t2.CodPostal,t2.NumRecinto");
...
try {
    $stmt = $Conexion->query("SELECT RutaFoto, NombreFoto FROM t_fotografias WHERE IdTarja = $IdTarja AND tipo = 3 and Almacen =$IdAlmacen ORDER BY IdFoto ASC");
    ...
    $html .= '
            ...
            <div class="text-overlay"><h4 class="remision"> REM-' . htmlspecialchars($IdRemision) . '</h4></div>
    ...
    header('Content-Disposition: inline; filename="TarjaIngreso_' . $Tarja .'.' .'.pdf"');
    echo $dompdf->output();
    exit; 
