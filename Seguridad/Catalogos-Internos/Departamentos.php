<?php
 ...
 $query = "SELECT max(IdDepartamento) as IdDepartamento FROM t_departamento;";
              if ($ress = mysqli_query($mysqli, $query)) {
                while ($fila = mysqli_fetch_assoc($ress)) {
                    $IdDepartamento = $fila["IdDepartamento"];        
              }
            }
              $CONT=$IdDepartamento+1; 
?>
<div class="container-fluid">
  <h1 style="text-align: right; ">Departamentos</h1>
    <button type="button" data-toggle="modal" data-target="#nuevoDepto" class="btn btn-primary btn-g" style="background-color:#808080; border-color:#808080;">
  ...
  <section class="pt-2">
    <div class="table-responsive">
      ...
      style="border-top-color: orange;">
        ...
        <tbody>
            <?php 
            $sql = "Select IdDepartamento, NomDepto from t_departamento";
                $result = mysqli_query($mysqli,$sql);
                while($row = mysqli_fetch_assoc($result)):
            ?>
            <tr>
                <td width="auto" style="text-align: center"><?php echo $ID=$row['IdDepartamento'];?></td>
                <td width="auto"style="text-align: center"><?php echo $row['NomDepto'];?></td>
                <td>
                  <button type="button" data-toggle="modal" data-target="#modificarDepto_<?php echo $ID; ?>_<?php  echo $usuario; ?>" class="btn btn-warning" >
                ...
                <?php 
                  include "mod_Departamento/Editar.php"; 
                  include "mod_Departamento/Alta.php"; 
                  endwhile;
                ?>
            </tr>
          </tbody>
...
function Confirmacionborrar(id,usuario,Mov) {
                              ...
                              swalWithBootstrapButtons.fire({
                                ...
                                if (result.isConfirmed) {
                                   ...
                                   $.ajax({
                                      url: 'mod_Departamento/procesos.php',
                                      type: 'POST',
                                      data: {
                                        id: id,
                                        usuario: usuario,
